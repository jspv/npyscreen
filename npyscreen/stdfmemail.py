...
class EmailViewFm(npyscreen.SplitFormWithMenus):
    ...
    def create(self):
        ...
        self.wDate = self.add(
            npyscreen.TitleText, begin_entry_at=10, editable=False, name="Date:"
        )

        self.draw_line_at = self.nextrely
        self.nextrely += 1
        _body_rely = self.nextrely
        self.wEmailBody = self.add(
            EmailPager, max_height=-1, scroll_exit=True, hidden=True
        )
        self.nextrely = _body_rely
        self.wMessageTree = self.add(
            EmailTree, max_height=-1, scroll_exit=True, hidden=False
        )
        self.nextrely += 1
    ...
    def _parse_email_tree(self, this_email):
        ...
        self._this_email_tree = npyscreen.NPSTreeData(
            content=this_email, ignoreRoot=False
        )
