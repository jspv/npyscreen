...
class MultiLineEdit(widget.Widget):
    ...
    def translate_cursor(self, y):
        ...
        if position == 0:
            return 0, 0
        text_to_cursor = self.get_value_as_list(
            upto=position, keepends=True, useEncoding=False
        )
        ...
        for line_counter in range(self.height):
            ...
            while column <= (display_width):
                ...
                if place_in_string >= len(line_to_display):
                    break
                width_of_char_to_print = (
                    1  # self.find_width_of_char(string_to_print[place_in_string])
                )
