        ...
        try:
            if self.value.month == 2 and self.value.day == 29:
                self.value = self.value.replace(
                    year=self.value.year - 1, day=self.value.day - 1
                )
            else:
        ...
        if self.value:
            ...
            try:
                ...
                self.parent.curses_pad.addstr(
                    self.rely + 1, self.relx, "Unable to display"
                )
                self.parent.curses_pad.addstr(
                    self.rely + 2, self.relx, "calendar for date."
                )
            if do_cal_print:
                # Print the day names
                ...
                if self.do_colors():
                    cal_title_attribute = self.parent.theme_manager.findPair(
                        self, "LABEL"
                    )
                else:
