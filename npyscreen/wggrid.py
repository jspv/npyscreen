...
class SimpleGrid(widget.Widget):
    ...
    def set_grid_values_from_flat_list(
        self, new_values, max_cols=None, reset_cursor=True
    ):
        if not max_cols:
    ...
    def make_contained_widgets(self):
        ...
        self._my_widgets = []
        column_width = (
            self.width + self.col_margin - self.additional_x_offset
        ) // self.columns
    ...
    def _print_cell(
        ...
        if (
            self.editing or self.always_show_cursor
        ) and cell.grid_current_value_index != -1:
    ...
    def h_scroll_right(self, inpt):
        if self.begin_col_display_at + self.columns < len(
            self.values[self.edit_cell[0]]
        ):
