...
class InputHandler(object):
    ...
    def handle_input(self, _input):
        ...
        if hasattr(self, "parent_widget") and hasattr(
            self.parent_widget, "handle_input"
        ):
...
class Widget(InputHandler, wgwidget_proto._LinePrinter, EventHandler):
    ...
    def __init__(
        ...
        **keywords
        ...
        self.interested_in_mouse_even_when_not_editable = (
            False  # used only for rare widgets to allow user to click
        )
    ...
    def clear(self, usechar=" "):
        ...
        for y in range(self.height):
            ...
            if self.do_colors():
                ...
                self.parent.curses_pad.addstr(
                    self.rely + y, self.relx, usechar * (self.width)
                )  # used to be width + 1
    ...
    def _get_ch(self):
        ...
        if ALLOW_NEW_INPUT is True and locale.getpreferredencoding() == "UTF-8":
            ch = self.parent.curses_pad.getch()
            if ch <= 127:
                rtn_ch = ch
    ...
    def get_and_use_key_press(self):
        global TEST_SETTINGS
        if (TEST_SETTINGS["TEST_INPUT"] is None) and (
            TEST_SETTINGS["INPUT_GENERATOR"] is None
        ):
            curses.raw()
            curses.cbreak()
            curses.meta(1)
    ...
    def safe_string(self, this_string):
        ...
        try:
            ...
            if self.__class__._SAFE_STRING_STRIPS_NL is True:
