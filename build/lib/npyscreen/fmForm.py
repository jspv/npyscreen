...
class _FormBase(
    proto_fm_screen_area.ScreenArea,
    widget.InputHandler,
    ...
    DEFAULT_X_OFFSET = 2
    PRESERVE_SELECTED_WIDGET_DEFAULT = (
        False  # Preserve cursor location between displays?
    )
    FRAMED = True
    ALLOW_RESIZE = True
    ...
    def find_mouse_handler(self, mouse_event):
        ...
        for wd in self._widgets__:
            try:
                if wd.intersted_in_mouse_event(mouse_event) is True:
                    return wd
            except AttributeError:
                pass
        return None
    ...
    def find_next_editable(self, *args):
        if not self.cycle_widgets:
            ...
            r = list(range(self.editw + 1, len(self._widgets__))) + list(
                range(0, self.editw)
            )
        for n in r:
    ...
    def add_widget(
        ...
        **keywords
        ...
        _w = widgetClass(
            self, rely=rely, relx=relx, max_height=max_height, *args, **keywords
        )
