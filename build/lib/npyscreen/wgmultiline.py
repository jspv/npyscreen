...
the same effect can be achieved by altering the __str__() method of displayed objects"""
    ...
    def update(self, clear=True):
        ...
        if not no_change or clear or self.never_cache:
            ...
            if self.editing or self.always_show_cursor:
                ...
                self._my_widgets[(self.cursor_line - self.start_display_at)].update(
                    clear=True
                )
            else:
        ...
        if not force_remake_cache:
            try:
                if (
                    ...
                    and self._last_values == self.values
                ):
    ...
    def move_next_filtered(self, include_this_line=False, *args):
        ...
        for possible in self._filtered_values_cache:
            if possible == self.cursor_line and include_this_line is True:
                self.update()
                break
        ...
        try:
            if (
                ...
                or self._my_widgets[self.cursor_line - self.start_display_at].task
                == MORE_LABEL
            ):
                if self.slow_scroll:
                    self.start_display_at += 1
    ...
    def handle_mouse_event(self, mouse_event):
        ...
        self.cursor_line = (
            rel_y // self._contained_widget_height + self.start_display_at
        )
    ...
    def h_cursor_line_down(self, ch):
        ...
        if (
            self._my_widgets[self.cursor_line - self.start_display_at].task
            == MORE_LABEL
        ):
            if self.slow_scroll:
                self.start_display_at += 1
        ...
        for r in range(len(self.values)):
            if hasattr(self.values[r], self.shortcut_attribute_name):
                ...
                if (
                    ...
                    or getattr(self.values[r], self.shortcut_attribute_name)
                    == _input_decoded
                ):
                    return r
        return False
